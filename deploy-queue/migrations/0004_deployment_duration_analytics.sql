...
-- Provides averages and standard deviation of deployment durations per component (per environment + cloud_provider + region + cell_index)
...
FROM (
    SELECT
        ...
        finish_timestamp,
        ROW_NUMBER() OVER (
            PARTITION BY environment, cloud_provider, region, cell_index, component
            ORDER BY id DESC
        ) as row_number
    FROM deployments
    WHERE
...
GROUP BY environment, cloud_provider, region, cell_index, component;

-- Indexes for efficient querying
CREATE INDEX idx_deployment_duration_analytics_component ON deployment_duration_analytics(component);
CREATE INDEX idx_deployment_duration_analytics_cell ON deployment_duration_analytics(environment, cloud_provider, region, cell_index);
CREATE INDEX idx_deployment_duration_analytics_component_cell ON deployment_duration_analytics(component, environment, cloud_provider, region, cell_index);
