...
latest_deployments AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            PARTITION BY deployment.region,
            deployment.component
            ORDER BY
                deployment.id DESC
        ) AS row_number
...
FROM
    (
        ...
        FROM
            deployments deployment
            LEFT JOIN deployment_duration_analytics analytics ON deployment.region = analytics.region
            AND deployment.component = analytics.component
            AND deployment.environment = analytics.environment
