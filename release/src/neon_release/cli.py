...
from neon_release import git, gh
...
def new(
    ...
    hotfix: Annotated[
        bool,
        typer.Option("--hotfix", help="Shortcut: set base to <release-branch>."),
    ] = False,
    ...
    auto_merge: Annotated[
        bool,
        typer.Option("--auto-merge", help="Enable auto-merge after PR approval."),
    ] = False,
    auto_approve: Annotated[
        bool,
        typer.Option("--auto-approve", help="Automatically approve the PR."),
    ] = False,
):
    ...
    try:
        pr_url = gh.create_pr(branch_name, base, pr_title, labels)
        typer.echo(f"[success] Created PR: {pr_url}")

        if auto_approve:
            pr_number = pr_url.rstrip("/").split("/")[-1]
            gh.approve_pr(pr_number)
